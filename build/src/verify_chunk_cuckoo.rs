...
/// Check whether `chunk_id` is in one of its assigned groups' cuckoo tables.
fn lookup_in_group(
    cuckoo_data: &[u8],
    group_id: usize,
    chunk_id: u32,
...
fn main() {
    ...
    for i in 0..verify_count {
        ...
        let assigned = derive_chunk_groups(chunk_id);
        ...
        if entry_found {
            ...
            if not_found <= 10 {
                eprintln!(
                    "  MISS: chunk {} (groups {:?}) at verify iteration {}",
                    chunk_id, assigned, i
                );
            }
        }
