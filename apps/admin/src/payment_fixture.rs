...
use pir_service_protocol::{
    derive_bat_key_id_v1, derive_cashu_keyset_id_v2, derive_provider_id, paid_receipt_key_id,
    AcquisitionMethod, AuthPaddingClassV1, AuthScheme, BackendId, Bolt11QuoteKeyDelegationV1,
...
fn build_provider_fixture(
    ...
    ensure_fixture_directory(root, &secrets_root)?;
...
mod tests {
    ...
    fn force_rejects_nested_directory_symlink_without_writing_outside_root() {
        ...
        .unwrap_err();
        assert!(error.contains("nested symlinks are forbidden"), "{error}");
        assert_eq!(
