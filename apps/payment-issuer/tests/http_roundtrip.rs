//! End-to-end HTTP test: spawn the `payment-issuer` binary, obtain an ARC
...
fn wait_until_ready() {
    ...
    panic!("payment-issuer did not become ready on port {PORT}");
}
...
#[test]
fn payment_issuer_full_obtain_present_verify_over_http() {
    // Fresh key file per run.
    let pid = std::process::id();
    let key_path = std::env::temp_dir().join(format!("payment_issuer_test_key_{pid}.bin"));
    let cashu_key = std::env::temp_dir().join(format!("payment_issuer_test_cashu_{pid}.bin"));
    let _ = std::fs::remove_file(&key_path);
    let _ = std::fs::remove_file(&cashu_key);

    let child = Command::new(env!("CARGO_BIN_EXE_payment-issuer"))
        .arg("--arc-key")
        .arg(&key_path)
        .arg("--cashu-key")
        .arg(&cashu_key)
