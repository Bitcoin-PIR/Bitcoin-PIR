//! Designated BitcoinPIR credential issuer (ARC + Cashu).
//!
//! This is the single issuer the PIR nodes announce. It issues and verifies
//! cashu/ARC credentials. Collection (Lightning) is not in this binary —
//! that rail is rebuilt later. Issuance is currently free: anyone who can
//! reach the HTTP surface can mint.
//!
//! HTTP/1.1 is hand-rolled over `std::net` so the offline `vendor/` tree is
//! not perturbed by a web framework.
//!
...
//! with `pir_runtime_core::arc_verifier::ArcVerifier`. The PIR server can be
//! launched with `--allow-experimental-arc --require-arc --arc-key <same path>`
//! and will verify credentials this issuer mints.
...
fn main() {
    ...
    while i < argv.len() {
        match argv[i].as_str() {
            ...
            "-h" | "--help" => {
                println!("payment-issuer — designated ARC/Cashu credential issuer");
                println!(
                    "usage: payment-issuer [--arc-key <path>] [--cashu-key <path>] [--port <n>]"
                );
                return;
            }
    ...
    println!("┌─────────────────────────────────────────────────────────────");
    println!("│ payment-issuer  (issue + verify; no Lightning collection)");
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(s) => {
                // BSD accept() can inherit O_NONBLOCK from the listener.
...
fn handle(mut stream: TcpStream, state: Arc<IssuerState>) {
    ...
    match (method.as_str(), path.as_str()) {
        ...
        ("GET", "/") | ("GET", "/health") => {
            write_response(&mut stream, 200, "OK", b"payment-issuer ok\n", "text/plain")
        }
