...
case "$lane" in
  core)
    cargo test --timings --locked --offline -p pir-channel -p pir-strict-https -p pir-private-files -p pir-service-protocol -p pir-service-store -p pir-payment-crypto -p pir-lightning-backend -p pir-issuer-store -p pir-issuer-core -p pir-issuer-credentials -p pir-issuer-clearing -p pir-issuer-service -p pir-provider-clearing-client -p pir-cashu-client -p pir-cashu-custody -p pir-arc-adapter -p pir-directory-nostr -p bitcoinpir-cln-rpc-guard -p pir-runtime-core -p pir-sdk-client -p pir-sdk-wasm -p payment-issuer -p bpir-admin
