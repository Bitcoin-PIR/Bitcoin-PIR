...
const REVIEWED_ONESHOT_UNITS = new Map([
  [
...
function validateUnitLifecycle(unit, properties, uptimeFinishedMilliseconds) {
  ...
  if (properties.Type === "oneshot") {
    const reviewedFragment = REVIEWED_ONESHOT_UNITS.get(unit.unit_name);
    if (
      reviewedFragment === undefined ||
      unit.fragment_path !== reviewedFragment ||
      canonicalJson(unit.hardening.Type ?? []) !== canonicalJson(["oneshot"]) ||
      !new Set(["yes", "true"]).has(unit.hardening.RemainAfterExit?.[0]) ||
      unit.hardening.RemainAfterExit?.length !== 1
    ) {
...
export function validateLiveRuntimeEvidence({
  ...
  for (let index = 0; index < request.units.length; index += 1) {
    ...
    for (const key of ["ExecStartPost", "ExecCondition", "EnvironmentFiles", "RootDirectory", "RootImage", "BindPaths", "LoadCredential", "SetCredential"]) {
...
export function collectLiveRuntimeEvidence({ bundleRoot, approvedManifestSha256, approvedPlanSha256, expectedMachineIdSha256 }) {
  ...
  // Per-unit checks inside collectUnit are not enough: a profile sentinel or
  // unit generation could otherwise change while later probes run. Recheck
  // structured Conditions and bind each same unit generation here, immediately
  // before the final host binding and evidence construction.
  for (let index = 0; index < request.units.length; index += 1) {
