...
case "$lane" in
  core)
    cargo fmt --all -- --check
    cargo test --timings --locked --offline -p pir-channel -p pir-private-files -p pir-runtime-core -p pir-sdk-client -p pir-sdk-wasm -p pir-credit -p bpir-admin
    cargo clippy --timings --locked --offline --all-targets --no-deps -p pir-private-files -p pir-credit -p bpir-admin -- -D warnings
    ;;
