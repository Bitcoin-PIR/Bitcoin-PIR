#!/usr/bin/env python3
"""
Fetch Bitcoin blocks from BlockCypher API and store as raw binary files.

Usage:
    python fetch_blocks_v2.py [--count N] [--test]
...
REQUEST_DELAY = 0.2  # seconds between requests (BlockCypher is more generous)
RATE_LIMIT_DELAY = 3.0  # seconds to wait after rate limit error
...
def fetch_latest_block() -> Optional[Dict[str, Any]]:
    """Fetch the latest block from BlockCypher."""
...
def extract_block_metadata(block_json: dict, filename: str) -> BlockMetadata:
    ...
    """
    # BlockCypher uses different field names than blockchain.info
    # Handle timestamp - BlockCypher returns ISO 8601 string
...
def fetch_blocks(count: int, verbose: bool = True) -> List[BlockMetadata]:
    ...
    if verbose:
        print(
            f"Latest block: height={current_height}, hash={latest_block.get('hash', '')[:16]}..."
        )
    ...
    for i in range(count):
        ...
        try:
            # Fetch block data from BlockCypher
            ...
            # Save block data as JSON bytes (PIR can work with any binary data)
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch Bitcoin blocks from BlockCypher API"
    )
    parser.add_argument(
        "--count",
        type=int,
    ...
    print("Bitcoin Block Fetcher v2 (BlockCypher API)")
    print("=" * 60)
