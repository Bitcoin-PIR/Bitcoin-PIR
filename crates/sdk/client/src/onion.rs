...
impl OnionClient {
    ...
    pub async fn enable_credits(
        ...
        let conn = self
            .conn
            .take()
            .ok_or_else(|| PirError::Protocol("enable_credits: server not connected".into()))?;
        let (conn, status) =
            crate::credit_transport::enable_credits(conn, provider, pir_credit::Backend::Onion)
                .await;
        self.conn = Some(conn);
        status
    }
    ...
    /// server and return its receipt. Bearer material: call this after the
