...
///
/// Every successful method must be durable before returning. `initialize`
/// installs an absent record and is idempotent only for the exact same initial
/// floor. `compare_and_advance` atomically changes `expected` to `next`, or
/// returns the already-current record without changing it. It must never
/// lower a generation, change a store identity, or accept two different
/// commitments at one generation.
