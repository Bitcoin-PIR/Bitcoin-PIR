...
mod tests {
    ...
    fn test_merkle_tree_non_power_of_2() {
        let leaves: Vec<Hash256> = (0..5u8).map(|i| sha256(&[i])).collect();
    ...
    fn test_verify_entry() {
        ...
        let proof_siblings: Vec<Hash256> = tree
            .proof(tree_loc as usize)
            .into_iter()
            .map(|(_, h)| h)
            .collect();
    ...
    fn test_tree_top_cache() {
        let leaves: Vec<Hash256> = (0..16u8).map(|i| sha256(&[i])).collect();
