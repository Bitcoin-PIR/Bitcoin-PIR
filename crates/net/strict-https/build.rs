...
fn main() {
    println!("cargo:rerun-if-env-changed=CARGO_FEATURE_TEST_ONLY_WEBPKI_ROOT");

    if env::var_os("CARGO_FEATURE_TEST_ONLY_WEBPKI_ROOT").is_none() {
        return;
    }
    ...
    if profile != "debug" {
        panic!("test-only-webpki-root must never be compiled into a production release");
    }
}
