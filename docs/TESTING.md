...
| UKI scripts, VPSBG operator scripts, `.github/workflows/**` | `node scripts/github-workflow-supply-chain-gate.mjs`; `node --test scripts/tier3-uki-policy-contract.test.mjs scripts/vpsbg-data-disk.test.mjs scripts/pir2-sealed-ceremony.test.mjs` (the second simulates the VPSBG control plane and sshd for `vpsbg-data-disk.sh open`; the third checks the ceremony wrapper's command previews, its `BPIR_ADMIN` prebuilt-binary override, and the exit status it forwards); `node scripts/vpsbg-tier3-generation.test.mjs` (runs the measured run script against fixtures, ~30 s); `bash scripts/vpsbg-production-status.test.sh`; `bash scripts/ops-operator-scripts.test.sh` | `workflow-supply-chain.yml` | UKI builds and live VPSBG mutations are operator actions, not CI |
