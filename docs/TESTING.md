...
| `crates/sdk/{core,client}` | `cargo test -p pir-sdk-client` (add `--features onion` if you touched the Onion path) | `pir-sdk-integration.yml` (deterministic jobs; live tests run only on schedule/dispatch; the `probe_live_credits_required` step reports which backends production charges for, and those backends' metered live steps skip themselves because CI holds no credential — see [Credits](CREDITS.md)) | — |
...
| Credits and gas (`crates/trust/pir-credit`, the server meter, opcode `0x12`, the clients) | `cargo test --locked --offline -p pir-credit`; `cargo test --locked --offline -p runtime --bin unified_server -- credit_` (meter, gate, issuer client with a loopback fake issuer); `cargo test --locked --offline -p pir-runtime-core --lib credit_wire`; `cargo test --locked --offline -p pir-sdk-client --lib credit` (presentation, gas card, classifier, credited transport against a fake server); `cargo test --locked --offline -p pir-sdk-wasm --lib credit` (ARC request → finalize → present against a local issuer key); `cd web && npx vitest run src/__tests__/credits.test.ts` | `rust-ci.yml` core and runtime lanes | Rate-card regression: `gas::tests::rate_card_2026_09_lands_on_ten_five_two_one` fails if a parameter or calibration change moves the headline prices ([Credits and gas](CREDITS.md)) |
...
| UKI scripts, VPSBG operator scripts, `.github/workflows/**` | `node scripts/github-workflow-supply-chain-gate.mjs`; `bash scripts/test-scripts.sh` (one `node --test` over every offline script suite: UKI policy contract, VPSBG data-disk open/close simulation, ceremony wrapper, rollback set, release-record generator, recovery-root receipt fetch, campaign dry-run plans; `--with-bash` adds `scripts/vpsbg-production-status.test.sh` and `scripts/ops-operator-scripts.test.sh`; `--list` prints the suites); `node scripts/vpsbg-tier3-generation.test.mjs` (runs the measured run script against fixtures, ~30 s) | `workflow-supply-chain.yml` | UKI builds and live VPSBG mutations are operator actions, not CI |
