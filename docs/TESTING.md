...
Payment V1 (signed policy, clearing, PoW, the admission gate) is deleted; the
former `scripts/payment-v1-local-check.sh` profiles no longer exist. Paid
queries present a single designated issuer's cashu/ARC credential on the
existing 0x08/0x09 opcodes; free queries are open.
