# db1 Free-PoW + BAT implementation plan (archived)
...
- Remaining work includes the executable production profiles and their later
  ...
  in
  [`MAINNET_SHARED_BAT_PRODUCTION_PLAN.md`](MAINNET_SHARED_BAT_PRODUCTION_PLAN.md).
  Nothing in the completed db1 policy/proof/UI source PR activates a provider,
  issuer, payment, database, UKI or public site.

## Historical source sequence and superseding boundary

The steps below record the completed db1 policy/proof/UI source slice. Any
...
issuer-wide plan, not evidence from this completed historical policy/admission
slice.
