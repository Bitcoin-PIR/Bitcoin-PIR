...
| Relay selection | Current state is `UNRESOLVED`. It passes only after exact relay source/commit, archive digest, `Cargo.lock`, binary, config, publisher-key pin, explicit strict-multi or centralized-single mode, failure-domain declaration and independent audit are recorded. Centralized mode records its degraded acceptance; strict origin count is not evidence of independent operators. | Any public directory listener, relay installation or catalog publication. | Production-key use, event publication or routing. |
| Directory publication | The relay-selection and fresh-live gates passed, and the exact signed non-secret provider events, relay destination(s), explicit directory mode, publisher key/action and mode-matched readback plan are approved. | Public Nostr event submission. | Advertising an inactive method/scope, DNS change or later event. |
...
| Relay selection | `UNRESOLVED` until source/commit/archive/lockfile/binary/config/publisher-key pins, explicit directory mode, failure-domain declaration and independent audit are recorded. | Directory publication cannot advance while this field is `UNRESOLVED`; centralized mode must remain visibly degraded. |
