...
- Commercial price and quota decisions live in signed offers/policies. This
  ...
  invent prices.
- The render gate accepts only the eight closed profiles listed below. A
...
| `integrated-existing-bhtm-caddy-v1` | Alternative append-only overlay for the exact existing root `bhtm-caddy.service`, plus source-fair HAProxy. It widens Payment V1 trust/failure/privacy scope to all existing Caddy global options, admin/ACME/journal state and sites; it is never evidence that those controls were hardened. | Exact Caddyfile preimage/binary/unit/active generation; managed-block/gate/executor/Node pins; content-addressed rename-exchange helper and manifest; HAProxy bundle/runtime pins; four distinct DNS names; public bind; distinct RFC1918/ULA publisher bind/client route; WebPKI publisher certificate/key; sealed transaction parents. The currently inspected host lacks that private route. | Separate remote host change; explicit existing-root-Caddy risk acceptance; private-route provisioning; private publisher ingress; source-fair fresh start; one reload transaction; public routing. | Approved render and overlay-plan digests; exact preimage/candidate/helper pins; append-only phase journal and atomically published terminal receipt; same Caddy PID/InvocationID/active generation; WebPKI+hostname+leaf and WebSocket-accept health proofs; separate cold-edge evidence. A warm overlay receipt cannot replace stopped-edge/fresh-start evidence. |
| `directory-publisher-netns-v1` | A no-key, no-retry one-shot publisher in a route-less named namespace. The current plan intentionally targets one centralized relay and does not claim relay redundancy or operator independence. | Content-addressed native namespace helper and `bpir-admin`; three frozen signed artifacts; one explicit private relay SNI/SAN and hosts entry; one pinned public directory key; exact namespace/veth policy; service UID/GID. Default CLI publication remains strict 2..8; this unit alone carries `--centralized-single-relay`. | Remote host change; private publisher ingress; SNI/SAN acceptance; UFW installation/reload; namespace activation; exact publication action. Publication is currently fail-closed because the required firewall-generation guard sentinel is intentionally unavailable. | Plan/manifest/runtime digests; nsfs/owner-generation evidence; IPv4/IPv6 forwarding disabled; UFW status/raw plus nft base/before/user-chain evidence before/after dry-run reload; shared Caddy `Wants`/`After` with no reverse `Requires`/`BindsTo`/`PartOf`; CLI receipt marked `centralized=true degraded=true`; independent relay readback. These snapshots are point-in-time only and cannot replace a reviewed wrapper or continuous monitor that binds the actual publication interval. |
