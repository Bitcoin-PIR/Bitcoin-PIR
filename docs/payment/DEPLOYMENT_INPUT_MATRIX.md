...
| --- | --- | --- | --- | --- |
| `core-pattern-ceremony-v1` | Host-global privacy maintenance prerequisite. It atomically persists and applies `kernel.core_pattern=|/usr/bin/false`, disables/stops Apport, and affects every process rather than only Payment V1. It never starts Caddy/Payment services, reboots, or deletes crash/journal history. | Canonical exact-current plan; boot/host/systemd/tool pins; exact Apport defaults/unit/service/core-pattern/sysctl-assignment closure; empty `/var/crash` inventory; exact executor source digest; rollback bytes. | Separate digest-bound application approval explicitly accepting host-wide loss of native core diagnostics; separate receipt-bound rollback approval to restore those diagnostics. | Canonical terminal receipt; exact root-owned persistent sysctl file; exact `enabled=0`; Apport inactive/disabled; immediate and final live readback; unchanged empty crash inventory. A contained/unknown state retains the host-global lock and cannot authorize edge activation. |
