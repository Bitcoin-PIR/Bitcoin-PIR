...
| Binary payload pins | SHA-256 of exact Caddy, HAProxy, Core, CLN bundle members, `bpir-admin`, guard, issuer, provider and rollback-authority bytes plus source/build provenance. | Build/release owner / binary reviewer. | Compute from staged regular files, not documentation. `CLN_BUNDLE_SHA256` identifies the independently approved upstream archive; the selected v26.06.6 closure is exactly 38 members, with only `bcli`/`chanbackup` executable among its 27 plugins and the other 25 exact-disabled, root-owned mode `0444`. It additionally binds exactly one `libpq.so.5` below the separate `CLN_LIBPQ_SHA256` digest-equals-file root, and the unit's sole loader assignment must select it. Every digest, mode, target path and one-entry/closed manifest must agree. Test or repeated-digit hashes are not production evidence. |
