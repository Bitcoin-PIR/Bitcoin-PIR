...
| --- | --- | --- | --- | --- |
| `core-pattern-ceremony-v2` | Host-global privacy prerequisite. It directly manages the exact Noble Apport enablement/mask state and all three Apport-mutated sysctls without executing stock ExecStart/ExecStop. | Canonical plan; official Noble Apport and systemd-sysctl unit/binary fixtures plus exact sysinit boot symlink; exact boot/host/tool/helper pins including busctl; three-sysctl assignment closure with matching glob/negative exclusions rejected; non-loading D-Bus unit enumeration plus complete Unit/Service `GetAll` values inside stable unit/job and static-load-path fences; exact Manager UnitPath, phase-specific credentials/drop-ins, closed control/transient/generated/administrator/vendor roots, quoted/escaped/specifier-expanded action dependencies, multi-level aliases and normalized handler `Exec*`; exact approval-bound lease, preflight intent, Apport/systemd-sysctl gates, reboot guard, mask, fixed temp/quarantine and `/var/crash` observations; exact closed lock-launcher argv/environment. | Fresh apply approval for the plan boot; fresh current-`/proc`-boot approval bound to the newest exact `lease`, `preflight`, or `pending` digest for every recovery attempt; separate receipt-bound rollback approval. | Exact terminal receipt with apply/action boot lineage, preflight digest and ordered nonterminal recovery-approval chain; full terminal-state comparison after candidate persistence and synchronous-with-publication final inspection; no-PID/no-job settled manager readback; exact safe three-sysctl readback; persistent policy, systemd credential closure and loaded Apport mask; cleanup runtime/full-state revalidation before lease release, then exact removal of enablement, transient gates, preflight, pending, lease and lock; final `/var/crash` recheck. Any unknown or retained-state outcome blocks activation. |
...
| `publisher-netns-ceremony-v1` | Initial same-host activation of the fixed no-key directory-publisher namespace; its receipt is scoped to one boot/systemd/nsfs generation. | Exact installed helper/units/drop-in/name-service/policy pins; exact executor and both local gate-import pins; regular descriptor-invoked Node/systemctl/ip pins; inactive netns/publisher preimages; exact Caddyfile/generation; five external sentinel pins; canonical UFW/nft evidence; boot/machine/systemd identity; fixed `10.203.0.0/30` topology; root-only transaction parents; explicit absence of a publisher private key. | Separate host mutation and namespace-start approval valid at most one hour. Rollback requires a different, fresh approval bound to the exact committed receipt. Neither approval authorizes Caddy or publication. | Canonical plan/approval digests; append-only phases; committed receipt binding veth aliases/MACs/indices, nsfs inode, exact functional plus inert-only interface set, connected routes, zero forwarding, unchanged Caddy/publisher and all closed inputs. Reboot requires fresh runtime evidence; point-in-time firewall evidence does not satisfy the publication-generation guard. |
...
VPSBG's existing measured provider is intentionally outside these eight render
...
| --- | --- | --- | --- |
| Source | Merged 40-hex commit, approved base, exact-head CI URLs/conclusions, deployment-template gate, rendered gate and runtime-evidence collector digests, plus Node/toolchain versions. | Release owner / security reviewer. | The external plan-approval record and every evidence record name the frozen source. The collector imports the rendered gate, which imports the deployment-template gate; all three sibling scripts must come from the same frozen commit. Schema-v1 plans instead bind exact template source hashes and payload bytes; they do not contain a source-commit field. A later source, gate or collector byte requires a new review and digest ceremony. |
