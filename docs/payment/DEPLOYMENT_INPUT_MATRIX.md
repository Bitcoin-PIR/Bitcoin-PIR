...
- Commercial price and quota decisions live in signed offers/policies. This
  ...
  invent prices.
- The render gate accepts only the ten closed profiles listed below. The
...
| `bitcoin-core-signet-v1` | Independent wallet-disabled default-Signet Bitcoin Core. It owns Core binaries/config/provenance, exact bitcoind UID/primary GID 52928, distinct cookie GID 52929 and `/srv/bitcoin`; it does not own Lightning or issuer state. | Externally approved Core release archive and extracted binary hashes; exact Guix-signature snapshot/threshold/fingerprints; canonical plan digest; locked NSS identities; outbound-network policy. | Remote stopped install; separate Core/default-Signet start and persistent chain-state approval. No wallet, test-coin, channel or issuer authority. | Offline plan/bundle verification; installed-byte/stopped-state checks; fresh v6 live evidence for exact unit/process/NSS, kernel Groups exclusion of cookie GID 52929 and `/srv/bitcoin`/`.cookie`; later exact chain/challenge/genesis readback. |
| `issuer-lightning-signet-v1` | Payment issuer, Core Lightning, CLN RPC guard and read-only default-Signet preflight. This domain knows invoices/payment hashes and must never receive PIR queries. Bitcoin Core is a separately approved dependency. | Exact CLN/admin/guard/issuer binaries and immutable CLN library closure; exact approved `bitcoin-core-signet-v1` manifest/root; default-Signet topology; Lightning payee key; quote delegation; issuer keys/policies; protected stores; four service identities; backup/restore records. | Remote host change; Signet identity/wallet creation; test-coin receipt; channel changes; custody approval; backup/restore approval; issuer activation. | Plan/manifest/runtime digests; default-Signet preflight; exact node/network/payee readback; CLN and external Core binary/config pins; guard deadman state; store and independent rollback-floor evidence; backup/restore rehearsal. |
...
VPSBG's existing measured provider is intentionally outside these ten render
...
| Stopped relay preparation | The `directory-relay-v1` plan binds the exact selection SHA-256. V2 stopped-relay evidence proves the sentinel-gated unit inactive/dead, config metadata exactly UID 52951/GID 52952/mode 0400 under a consumer-owned 0700 final parent, effective `ProtectProc=invisible` and `ProcSubset=pid`, real-EUID access, typed `a(sbbsi)` Conditions with `RELAY-SELECTION-RESOLVED` absent, and—when resolved—the exact binary, both one-entry manifests, config and fragment installed-file closure. | Review of the installed resolved-but-stopped generation after separately approved installation; it never permits a start. | Creating any activation sentinel, live evidence, publisher-private-key use, listener start, routing or publication. |
...
| Stopped relay preparation | Full-file v2 stopped-relay evidence SHA-256 and offline verification result. | The sole resolved relay unit is loaded but inactive/dead; typed `a(sbbsi)` Conditions prove all startup prerequisites are not satisfied and `RELAY-SELECTION-RESOLVED` is absent; binary, both manifests, config and fragment match the manifest; config UID/GID/mode are 52951/52952/0400 and it is consumer-readable under a descriptor-bound 0700 final parent; effective `ProtectProc=invisible` and `ProcSubset=pid`; no protected UID/GID holder exists. |
