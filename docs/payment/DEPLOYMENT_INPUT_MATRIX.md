...
| Stopped relay preparation | The schema-v2 `directory-relay-v1` plan binds the exact selection SHA-256 and exact `systemd 255 (255.4-1ubuntu8.15)` build. V4 stopped-relay evidence proves the sentinel-gated unit inactive/dead, config metadata exactly UID 52951/GID 52952/mode 0400 under a consumer-owned 0700 final parent, effective `ProtectProc=invisible` and `ProcSubset=pid`, real-EUID access, typed `a(sbbsi)` Conditions with `RELAY-SELECTION-RESOLVED` absent, typed `a(sasasttttuii)` `ExecStartEx`/`ExecStartPreEx` matching approved path/argv/flags, all five typed Service credential arrays empty, and—when resolved—the exact binary, both one-entry manifests, config and fragment installed-file closure. | Review of the installed resolved-but-stopped generation after separately approved installation; it never permits a start. | Creating any activation sentinel, live evidence, publisher-private-key use, listener start, routing or publication. |
