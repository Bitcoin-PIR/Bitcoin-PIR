...
| Production mainnet | Source-only review and the focused browserless Mainnet V1 check are permitted. A private materialized plan and any host action require their own approvals. | No target-host preflight, Mainnet CLN node, liquidity/funds, remote installation, activation, invoice/payment or live evidence is implied by the source profile. |
...
| `issuer-lightning-mainnet-v1` | Source-ready Direct BOLT11/DPF, ledger-only Mainnet issuer, CLN RPC guard and read-only local Core/CLN preflight. It has no Cashu, ARC or payout surface. | Approved non-secret issuer/payee IDs, local Core/CLN paths and pins, profile/receipt/delegation hashes, service IDs, risk caps, selected Mainnet CLN and liquidity/funding envelope. | Source freeze; private plan digest; remote host mutation; custody/identity/backup restoration; Mainnet liquidity/funds; activation; any later invoice/payment action. | A private complete plan, rendered bundle, target-host read-only preflight, CLN/Core/pin/backup/liquidity readbacks and later separately approved live evidence. The checked-in skeleton is intentionally incomplete and cannot render. |
...
VPSBG's existing measured provider is intentionally outside these eleven render
...
| Mainnet source readiness | Versioned profile, read-only preflight, negative tests and source/render/Web contracts exist. `scripts/payment-v1-mainnet-lightning-v1-check.sh` is the limited browserless entry. | Private materialized plan or remote action until the separate input and approval records are complete. | Target-host readiness, a Mainnet CLN node, liquidity/funds, install, activation, invoice/payment, route, canary or service claim. |
