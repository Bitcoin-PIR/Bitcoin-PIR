...
| Stopped relay preparation | The `directory-relay-v1` plan binds the exact selection SHA-256. V2 stopped-relay evidence proves the sentinel-gated unit inactive/dead, config metadata exactly UID 62951/GID 62952/mode 0400 under a consumer-owned 0700 final parent, real-EUID access, typed `a(sbbsi)` Conditions with `RELAY-SELECTION-RESOLVED` absent, and—when resolved—the exact binary, both one-entry manifests, config and fragment installed-file closure. | Review of the installed resolved-but-stopped generation after separately approved installation; it never permits a start. | Creating any activation sentinel, live evidence, publisher-private-key use, listener start, routing or publication. |
