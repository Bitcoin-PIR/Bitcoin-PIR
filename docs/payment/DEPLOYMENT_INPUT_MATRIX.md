...
| Production Cashu mint selection | Exact production origin, WebPKI/leaf pins, unit, public keysets, custody/recovery epochs, exposure caps and outage/retirement policy are accepted. If mint-dependent current and retained offers are omitted, record that omission and do not materialize or activate the current `provider-v1` profile: its closed unit and skeleton always carry Standard-Cashu custody/recovery/exposure inputs. A provider without Standard Cashu needs a separately reviewed profile and negative tests. Local CDK/fake/loopback mints do not satisfy this gate. | Materializing mint-dependent provider fields, rendering the current `provider-v1` profile or advertising a Standard Cashu offer. | Contacting the mint, transferring value or public publication. |
...
| Cashu mint | Production selection approval and exact origin/pins/unit/keysets/custody/exposure evidence, or an explicit record that mint-dependent current and retained offers were omitted and the current `provider-v1` profile was not materialized or activated. | Local CDK/fake/loopback evidence cannot satisfy the production selection. External mint contact also requires its separate approval; omission requires a future separately reviewed no-Standard-Cashu profile before such a provider can start. |
