...
| Stopped relay preparation | The `directory-relay-v1` plan binds the exact selection SHA-256. V3 stopped-relay evidence proves the sentinel-gated unit inactive/dead, config metadata exactly UID 52951/GID 52952/mode 0400 under a consumer-owned 0700 final parent, effective `ProtectProc=invisible` and `ProcSubset=pid`, real-EUID access, typed `a(sbbsi)` Conditions with `RELAY-SELECTION-RESOLVED` absent, all five typed Service credential arrays empty, and—when resolved—the exact binary, both one-entry manifests, config and fragment installed-file closure. | Review of the installed resolved-but-stopped generation after separately approved installation; it never permits a start. | Creating any activation sentinel, live evidence, publisher-private-key use, listener start, routing or publication. |
...
| Stopped relay preparation | Full-file v3 stopped-relay evidence SHA-256 and offline verification result. | The sole resolved relay unit is loaded but inactive/dead; typed `a(sbbsi)` Conditions prove all startup prerequisites are not satisfied and `RELAY-SELECTION-RESOLVED` is absent; all five Service credential properties have their exact typed empty arrays; binary, both manifests, config and fragment match the manifest; config UID/GID/mode are 52951/52952/0400 and it is consumer-readable under a descriptor-bound 0700 final parent; effective `ProtectProc=invisible` and `ProcSubset=pid`; no protected UID/GID holder exists. |
