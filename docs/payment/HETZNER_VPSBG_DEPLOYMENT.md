...
the rollback removal re-proof. Every `apport-coredump-hook` and
`systemd-coredump` family name across all systemd 255 unit types fails closed,
including `.path`, `.timer`, `.socket`, implicit same-name service/template
