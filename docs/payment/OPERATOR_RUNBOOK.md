...
each database in an effective-user-owned mode-0700 directory and keep the main
database file mode 0600. Serving startup rejects a configured final-component
symlink, non-regular file, wrong owner, broader mode, non-private canonical
parent, or store/authority aliases (including the same inode through hard
links). The private parent is also the protection boundary for SQLite's
runtime `-wal` and `-shm` sidecars; do not chmod those sidecars public, move
them into a shared directory, or back them up under weaker access controls.
...
system. Multi-host active/active requires a reviewed linearizable store and
failover protocol.
...
2. validate the implemented global connection/auth semaphores,
   ...
   deadline (`--service-pre-auth-timeout-ms`, which Ping cannot extend), the
   512 KiB frame/message cap, and 16 MiB per-request / 64 MiB process-wide
...
- Outcome unknown during an external mint/issuer mutation: reconcile only the
  exact idempotent transcript. Never create fresh outputs/credentials.
...
sizes. `--unsafe-debug-query-logging` is a privacy-dangerous local diagnostic
escape hatch and prints an unmistakable startup warning. Never enable it in
production; its output must use isolated short retention and must never be
joined with payment/issuer logs.
