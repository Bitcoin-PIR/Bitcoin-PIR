...
earlier draft implemented a shared issuer with provider-specific BATs, twelve
raw BAT key lineages and provider-side payment stores. That shape is
superseded by this plan and must not be rendered or activated. The independent
pir1 Harmony db0/db1 multi-pool work remains useful.
...
The current draft contains two different kinds of work:

- **retain:** pir1 exact-db Harmony multi-pool routing, cross-db V2Half token
  isolation and their narrow tests;
...
- useful Harmony multi-pool work is separated from superseded payment work;
...
selection model. Preserve the already-correct db0/db1 roots and Harmony routing
work.
