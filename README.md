...
> **Trust-model note on Merkle verification (2026-06).** The root the client checks proofs against is currently **supplied by the same server** (in its info / tree-tops responses); the client does not yet compare it to the attested `manifest_roots`. A passing proof therefore establishes *self-consistency* — the server is held to one committed database, and corrupted, truncated, or version-skewed data is rejected — but a fully malicious server could commit to a forged database and still pass. Integrity against that server comes from **remote attestation**: the demo servers run a reproducibly built, pinned binary that verifies its database manifest at load. On pir2 the pin is a hardware-backed SEV-SNP measurement; pir1 has no SEV hardware, so its binary hash is self-reported and integrity there reduces to trusting the operator. Attestation results are currently surfaced to the user (advisory) rather than enforced fail-closed. Closing the loop — anchoring the Merkle root to the attested manifest roots and refusing to verify on mismatch, as an opt-in strict mode — is tracked in [docs/CODE_REVIEW_2026-06.md](docs/CODE_REVIEW_2026-06.md) ("Follow-up: strict verification mode").
