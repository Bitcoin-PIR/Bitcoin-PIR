...
> **Privacy note on Merkle verification (post-closure).** Within each PIR round, queries are padded to a fixed count (75 for index, 80 for chunk) so the server cannot tell which group is real. As of 2026-04-29 the *number of rounds* is also constant: every query (found / not-found / whale) emits exactly the same wire shape — same INDEX rounds, same CHUNK rounds, same number of Merkle sibling passes. The found-vs-not-found leak the older trade-off documented is closed via two invariants: **INDEX Merkle Group-Symmetry** (PBC-plan-based INDEX batching forces `max_items_per_group_per_level = 2` regardless of input collision pattern) and **CHUNK Merkle Item-Count Symmetry** (every query contributes exactly `M = 16` chunk Merkle items via `pad_chunk_ids_to_m`, real + synthetic). Empirical witness: a found and a not-found query produce **byte-identical** leakage profiles on every backend (DPF, HarmonyPIR, OnionPIR). See [docs/VERIFICATION_OVERVIEW.md](docs/VERIFICATION_OVERVIEW.md) for the full picture.
