...
interface PirSdkWasm {
  ...
  /** Transport-free V1 admission codec/verifier for standalone OnionPIR. */
  WasmStandaloneOnionServiceAdmissionV1: {
    new(
      dbId: number,
      secureChannelExporter: Uint8Array,
    ): WasmStandaloneOnionServiceAdmissionV1;
  };
  ...
  /** Restore an opaque, encrypted-vault-authenticated BOLT11 acquisition. */
  ...
  /** Exact 16-shard NIP-01 REQ messages generated by Rust. */
  directoryFullCatalogReqJsonV1(pinnedDirectoryPubkey: Uint8Array): string;
  /** Canonical empty anti-rollback checkpoint. */
  initialServicePolicyCheckpointV1(): Uint8Array;
  /** Canonical empty quote-key checkpoint, bound to one issuer/network/payee. */
  initial_bolt11_quote_key_checkpoint_v1(
    issuerId: Uint8Array,
    network: WasmLightningNetworkV1,
...
/** One-provider, one-socket OnionPIR admission state. It never owns the
 * WebSocket and never learns another provider identity. */
export interface WasmStandaloneOnionServiceAdmissionV1 {
  free(): void;
  policyRequest(): Uint8Array;
  retainedPolicyRequest(policyDigest: Uint8Array): Uint8Array;
  verifyPolicySession(policy: WasmAcceptedServicePolicyV1): void;
  verifyRetainedPolicySession(
  ...
  acceptRetainedAuthorizationResponse(
    responseFrame: Uint8Array,
    policy: WasmAcceptedRetainedServiceRedemptionV1,
    nowUnix: bigint,
  ): ServiceGrantViewV1;
...
export interface ServiceOfferViewV1 {
  offerId: number;
  acquisition: 'free' | 'bolt11' | 'cashu-ecash';
  authorization:
    | 'free'
  ...
  freeMode:
    ...
    | 'proof-of-work'
    | 'anonymous-ticket';
  verification: 'provider-local' | 'shared-issuer-online' | 'standard-cashu-mint-online';
  deploymentStatus: 'stable' | 'experimental';
  priorityClass: number;
  price: { kind: 'free' } | { kind: 'msat' | 'cashu'; amount: string; unit?: string };
  issuerIdHex: string;
  keyIdHex: string;
...
export interface ServiceScopeViewV1 {
  scopeIdHex: string;
  backend: 'dpf-pir' | 'harmony-pir' | 'onion-pir' | 'tee-oram';
  workload: 'dpf-query' | 'harmony-hint' | 'harmony-query' | 'onion-session' | 'tee-oram-query';
...
export interface ServiceEntitlementLimitsViewV1 {
  maxLogicalInputs: number;
  maxFrames: number;
  maxRequestBytes: string;
  maxResponseBytes: string;
  maxWallTimeMs: number;
  maxConcurrentSockets: number;
  maxHintGroups: number;
  maxWorkUnits: string;
}
...
export interface ServicePolicyViewV1 {
  providerIdHex: string;
  policyDigestHex: string;
  policyEpoch: string;
  expiresAtUnix: string;
  scopes: ServiceScopeViewV1[];
}

export interface ServiceGrantViewV1 {
  scopeIdHex: string;
  enforcedProfile: number;
  expiresInMs: number;
  hasHarmonyAttach: boolean;
}

/** Opaque live policy handle; valid only for its current secure connection. */
export interface WasmAcceptedServicePolicyV1 {
  free(): void;
  ...
  checkpointBytes(): Uint8Array;
  acknowledgeCheckpointPersisted(): void;
  validateAuthorizationProof(
    scopeId: Uint8Array,
    offerId: number,
    proofBytes: Uint8Array,
  ): void;
  /** Parse and bind one standard Cashu V3/V4 token without mint I/O. */
  ...
  offersJson(): ServicePolicyViewV1;
  /** Begin acquisition for one exact offer in this verified policy. */
  beginBolt11Acquisition(
    scopeId: Uint8Array,
    offerId: number,
 ...
 * provider/policy/scope/offer credential redemption on the current channel. */
export interface WasmAcceptedRetainedServiceRedemptionV1 {
  free(): void;
  readonly providerIdHex: string;
  readonly policyDigestHex: string;
...
export type WasmBolt11QuoteStatusV1 =
  | 'invoice-open'
  | 'payment-settled'
  | 'credential-claimed'
  | 'invoice-expired-pending-reconcile'
  | 'late-settled-reconcile';

/**
 * Secret-bearing BOLT11 acquisition state. Persist only its encrypted
 * `recovery_state_bytes()` output; none of these values belong on PIR wire.
 */
export interface WasmBolt11AcquisitionV1 {
  free(): void;
  quote_intent_bytes(): Uint8Array;
  quote_key_checkpoint_bytes(): Uint8Array;
  recovery_state_bytes(): Uint8Array;
  accept_initial_quote(quoteBytes: Uint8Array, nowUnix: bigint): void;
  invoice(): string;
  quote_id_hex(): string;
  quote_status(): WasmBolt11QuoteStatusV1;
  invoice_expires_at_unix(): string;
  claim_deadline_unix(): string;
  build_status_request(requestedAt: bigint): Uint8Array;
...
export interface WasmIssuedCapabilitiesV1 {
  free(): void;
  readonly scheme: 'bolt11-direct-receipt' | 'cashu-bat' | 'arc-experimental';
  count(): number;
  capability(index: number): Uint8Array;
}

/**
 * Verified directory candidate. `selectableCatalogJson()` remains unavailable
 * until the encrypted durable adapter commits and acknowledges the exact CAS
 * successor returned by `prepareRollback()`.
 */
export interface WasmDirectoryCatalogCandidateV1 {
  free(): void;
  stateKeysJson(): string;
  prepareRollback(currentStateJson: Uint8Array): string;
  acknowledgePersisted(durableStateJson: Uint8Array): void;
  selectableCatalogJson(): string;
}

/** Verified provider/channel/operation-bound free PoW challenge. */
export interface WasmServicePowChallengeV1 {
  free(): void;
  readonly difficultyBits: number;
  readonly expiresAtUnix: string;
  /** Empty result means this bounded nonce range had no solution. */
...
export interface WasmDpfClient {
  ...
  fetchRetainedServiceRedemption(
    serverIndex: number,
    dbId: number,
    expectedProviderId: Uint8Array,
    policySigningKey: Uint8Array,
 ...
 * round-trip the full state through the encrypted IndexedDB admission vault
 * so a page reload restores the remaining-presentation count without
 * re-fetching the credential from the payment service.
 */
