...
export function directoryRefreshFailureStateV1(
  ...
  const expiry = catalog?.catalogValidUntilUnix;
  const current = typeof expiry === 'string' && /^[1-9][0-9]*$/.test(expiry)
    && nowUnix > 0n && nowUnix <= BigInt(expiry);
  if (current
  ...
  return {
    retainCatalog: false,
    statusText: 'Directory refresh rejected; no previously verified directory catalog remains usable. Admission stays fail closed and does not fall back to manual trust; there is no automatic retry.',
  };
}
