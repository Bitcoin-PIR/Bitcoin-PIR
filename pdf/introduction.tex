...
This paper presents a complete system for private UTXO lookups. Section~\ref{sec:data} describes the Bitcoin UTXO model, our data representation, and techniques for reducing the UTXO set size. Section~\ref{sec:batching} presents the batch PIR framework using Probabilistic Batch Codes, which is PIR-variant-agnostic and applies across all protocol choices. Section~\ref{sec:2server} details the 2-server stateless DPF-PIR instantiation, including background on distributed point functions and our complete implementation.
