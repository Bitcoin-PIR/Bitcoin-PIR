...
\textbf{Level 2: Chunk retrieval.} Using the chunk locations from Level~1, the client repeats the process with $K_\text{chunk}{=}80$ PBC buckets over the chunk database. The chunk cuckoo uses 2 hash functions with bucket size 3, so the client generates 2 DPF key pairs per bucket. The recovered 132-byte results (3 slots $\times$ 44 bytes each) contain chunk identifiers and full UTXO records: 32-byte TXIDs, vout indices, and amounts in satoshis---everything needed to construct a spending transaction.
