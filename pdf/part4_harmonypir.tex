...
That is, each database row $k$ is assigned to segment $s = \lfloor P(k)/T \rfloor$ by the PRP, and the hint for segment $s$ is the XOR of all rows assigned to it. Rows in the range $[N, N')$ are virtual zero-rows and do not affect the XOR. The total hint size is $M \times w$ bytes per bucket.

\textbf{Per-bucket hints.} The system maintains 155 independent HarmonyPIR instances---one per PBC bucket (75 index-level + 80 chunk-level). Each instance uses a distinct PRP key derived from a shared master key by XORing the bucket identifier into bytes 12--15 of the key. The hint server computes and streams hints for all 155 buckets, using batch parallelism for efficiency.
...
To prevent the server from distinguishing real queries from padding, the client generates \emph{synthetic dummy} queries for PBC buckets that do not hold a real query in a given round.
...
HarmonyPIR plugs into the same PBC framework described in Section~\ref{sec:batching}. The client maintains 155 independent HarmonyPIR bucket instances:
\begin{itemize}[nolistsep,leftmargin=*]
    \item \textbf{Index level:} $K{=}75$ buckets, each with entry size $w = 39$ bytes (3 cuckoo slots $\times$ 13 bytes per index entry).
    \item \textbf{Chunk level:} $K_\text{chunk}{=}80$ buckets, each with entry size $w = 132$ bytes (3 cuckoo slots $\times$ 44 bytes per chunk slot).
\end{itemize}

The query protocol proceeds in rounds, analogous to DPF-PIR:
\begin{enumerate}[nolistsep,leftmargin=*]
    \item The client places $m$ ScriptPubKey queries into $K$ PBC buckets via cuckoo hashing (same as Section~\ref{sec:batching}).
    \item For each of the 2 cuckoo hash function rounds at the index level:
    \begin{itemize}[nolistsep,leftmargin=*]
        \item Buckets with a real query generate a HarmonyPIR request targeting the assigned cuckoo bin.
        \item Buckets without a real query generate a synthetic dummy.
    \end{itemize}
    \item All $K$ requests are sent to the query server in a single batch. The server responds with $K$ XOR results.
    \item The client processes responses: real queries recover index entries via the hint XOR; dummy responses are discarded.
    \item The process repeats for the chunk level with $K_\text{chunk}$ buckets.
\end{enumerate}
...
Each HarmonyPIR bucket instance supports approximately $M \approx N/T \approx \sqrt{N/2}$ queries before its hints are exhausted. For a typical bucket with $N \approx 755{,}000$ rows, this gives $M \approx 616$ queries per bucket.

To avoid re-downloading hints unnecessarily, the client caches hint state:
\begin{itemize}[nolistsep,leftmargin=*]
    \item \textbf{State serialization:} Each bucket's full state (hints, PRP key, relocation structure, query counter) can be serialized and stored in the browser's \texttt{localStorage} or on disk.
    \item \textbf{Exhaustion detection:} Before each batch query, the client checks the minimum remaining query budget across all active buckets. If any bucket is exhausted, the client triggers a hint refresh from the hint server.
