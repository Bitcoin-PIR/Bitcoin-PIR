...
\hline
\textbf{Database} & \textbf{Entry Size} & \textbf{Buckets ($K$)} & \textbf{PIR domain ($N$)} \\
\hline
Index (Level 1) & 28 B & 75 & $2^{20}$ \\
Chunks (Level 2) & 256 B & 80 & $2^{20}$ \\
\hline
\end{tabular}
\caption{Batch PIR parameters. Each level uses $K$ parallel PIR queries over tables of $N$ entries.}
\label{tab:databases}
\end{table}
