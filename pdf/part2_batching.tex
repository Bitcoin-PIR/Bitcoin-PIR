...
\textbf{Level 1: Index lookup.} The client computes RIPEMD-160 hashes of all queried ScriptPubKeys, derives $h$ candidate buckets per query via cuckoo hash functions, and places them into $K$ PBC buckets. For each bucket, the client generates a PIR query targeting the assigned row in the index table and sends it to the server(s). The responses are combined to recover index entries containing chunk IDs and data lengths.

\textbf{Level 2: Chunk retrieval.} Using the chunk locations from Level~1, the client repeats the process with $K_\text{chunk}$ PBC buckets over the chunk database. The recovered chunks contain full UTXO records: 32-byte TXIDs, vout indices, and amounts in satoshis---everything needed to construct a spending transaction.
...
\hline
\textbf{Database} & \textbf{Entry Size} & \textbf{Buckets ($K$)} & \textbf{Cuckoo} & \textbf{DPF queries/bucket} & \textbf{PIR domain} \\
\hline
Index (Level 1) & 26 B & 75 & 2-hash, bs=3 & 2 & $2^{20}$ \\
Chunks (Level 2) & 80 B & 80 & 3-hash, bs=2 & 3 (or 1$^*$) & $2^{21}$ \\
\hline
\end{tabular}
\caption{Batch PIR parameters. Each level uses $K$ parallel PIR queries over cuckoo hash tables. ($^*$For single-chunk addresses, the placement optimization in Section~\ref{sec:placement} reduces chunk DPF queries from 3 to 1 per bucket.)}
\label{tab:databases}
\end{table}
