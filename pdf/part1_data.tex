...
\textbf{Chunk map.} Stores the actual UTXO data in fixed-size entries (default 256 bytes). Full 32-byte TXIDs are stored directly in the chunks, eliminating a separate TXID resolution step. The chunk map is also organized as a cuckoo hash table for PIR-compatible retrieval.
