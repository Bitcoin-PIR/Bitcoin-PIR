...
\textbf{Index map.} Maps each ScriptPubKey (via its RIPEMD-160 hash) to the chunk location and data length. This is organized as a cuckoo hash table for $O(1)$ keyword-to-index lookup \cite{PR01}. Each index entry is 26 bytes: the 20-byte script hash (for verification), a 4-byte chunk offset, a 1-byte chunk count, and a 1-byte flags field (used for the placement optimization described in Section~\ref{sec:placement}).

\textbf{Chunk map.} Stores the actual UTXO data in fixed-size 80-byte blocks. Full 32-byte TXIDs are stored directly in the chunks, eliminating a separate TXID resolution step. The chunk map is also organized as a cuckoo hash table for PIR-compatible retrieval.
