...
/// TXID Mapping Database with custom murmurhash3-style hash functions.
///
/// This database maps 4-byte TXIDs to 32-byte TXIDs using cuckoo hashing
/// with DIFFERENT hash functions than the standard cuckoo database.
///
/// Database format:
/// - Each entry: 4-byte key (4b TXID) + 32-byte value (32b TXID) = 36 bytes
/// - Each bucket: 4 entries
/// - Total buckets: 30,097,234
pub struct TxidMappingDatabase {
    /// Database ID
    id: String,
    /// Path to the database file
    data_path: String,
...
impl TxidMappingDatabase {
    /// Create a new TxidMappingDatabase
    pub fn new(
        id: impl Into<String>,
        data_path: impl Into<String>,
        num_buckets: usize,
    ...
    /// Create a new TxidMappingDatabase with memory mapping
    pub fn with_mmap(
        id: impl Into<String>,
        data_path: impl Into<String>,
        num_buckets: usize,
...
impl Database for TxidMappingDatabase {
    fn id(&self) -> &str {
        &self.id
    }
    ...
    /// Custom hash function 1: murmurhash3-style finalizer
    /// Key is interpreted as little-endian u32
    fn hash1(&self, key: &[u8]) -> usize {
        // Interpret key as u32 (little-endian)
        let key_u32 = if key.len() >= 4 {
        ...
        h ^= h >> 16;
        h as u64 as usize % self.num_buckets
    }

    /// Custom hash function 2: different mixing constants
    /// Key is interpreted as little-endian u32
    fn hash2(&self, key: &[u8]) -> usize {
        // Interpret key as u32 (little-endian)
        let key_u32 = if key.len() >= 4 {
    ...
    fn read_bucket(&self, bucket_idx: usize) -> Result<Vec<Vec<u8>>, String> {
        ...
        let bucket_bytes = self.bucket_bytes(); // 4 * 36 = 144 bytes
        let offset = bucket_idx * bucket_bytes;
        ...
        if let Some(ref mmap) = self.mmap {
            for i in 0..4 {
                let entry_offset = offset + i * 36;
                if entry_offset + 36 <= mmap.len() {
                    let entry = mmap[entry_offset..entry_offset + 36].to_vec();
                    entries.push(entry);
                } else {
                    entries.push(vec![0u8; 36]);
                }
            }
        } else {
            let mut file = File::open(&self.data_path)
...
/// Compute cuckoo hash locations for a u32 key using the TXID mapping hash functions.
/// Returns (location1, location2) tuple.
#[inline(always)]
pub fn txid_mapping_locations(key: u32, num_buckets: usize) -> (usize, usize) {
    // Hash function 1: murmurhash3-style finalizer
    let mut h1 = key;
    h1 ^= h1 >> 16;
    h1 = h1.wrapping_mul(0x45d9f3b);
    h1 ^= h1 >> 16;
    let loc1 = h1 as u64 as usize % num_buckets;

    // Hash function 2: different mixing constants
    let mut h2 = key;
    h2 ^= h2 >> 15;
    h2 = h2.wrapping_mul(0x735a2d97);
    h2 ^= h2 >> 15;
    h2 = h2.wrapping_mul(0x0bef6c35);
    h2 ^= h2 >> 16;
    let loc2 = h2 as u64 as usize % num_buckets;
